...
def tokenize_row(feature, model: Optional[Union[PreTrainedModel, nn.Module]] = None, truncation_mode="keep_start", max_length=512, max_prompt_length=256) -> Dict:
...
def get_data_device_iterator(config, tokenizer, num_proc: int = 1):
    ...
    ds = ds.map(tokenize_row, num_proc=num_proc, max_prompt_length=config.max_prompt_length, max_length=config.max_length)
